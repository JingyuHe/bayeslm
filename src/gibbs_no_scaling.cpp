...
// [[Rcpp::export]]
List hs_gibbs_no_scaling(arma::mat Y, arma::mat X, int nsamps = 1000, double a = 1, double b = 1){
...
inline arma::mat sampling_beta(arma::mat mu_n, arma::mat chol_Lambda_n_inv, double sigma, int p){
    ...
    arma::mat output = chol_Lambda_n_inv * eps + mu_n;
    return output;
...
inline double sampling_tau(arma::mat lambda, arma::mat beta, double sigma, double tau){
    ...
    double mu2_tau = as_scalar(arma::sum(pow(beta / (lambda), 2), 1));
