    ...
    */
    ...
    while (h < nsamps)
    {
        ...
        for(int i = 0; i < N_blocks; i ++){
            ...
            priorcomp = log_horseshoe_approx_prior(b.rows(block_indexes(i), block_indexes(i+1)-1) , s * vglobal, penalize.rows(block_indexes(i), block_indexes(i+1)-1))- log_normal_density_matrix(b.rows(block_indexes(i), block_indexes(i+1)-1), arma::diagmat(eta.rows(block_indexes(i), block_indexes(i+1)-1)) / pow(s, 2), singular);
            ...
            if(i == 0 && icept == true){
                ...
                while (log_horseshoe_approx_prior(beta_hat_block + betaprop, s * vglobal, penalize.rows(block_indexes(i), block_indexes(i+1)-1)) - log_normal_density_matrix(beta_hat_block + betaprop, arma::diagmat(eta.rows(block_indexes(i), block_indexes(i+1)-1)) / pow(s,2), singular) < ly){
        ...
        ratio = exp(log_horseshoe_approx_prior(b, s * vgprop, penalize) - log_horseshoe_approx_prior(b, s * vglobal, penalize) + log(vgprop) - log(vglobal));
