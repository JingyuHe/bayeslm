    ...
    */
    ...
    {   
        ...
        for(int i = 0; i < N_blocks; i ++){
            ...
            priorcomp = log_asymmetric_prior(b.rows(block_indexes(i), block_indexes(i+1)-1), vglobal, prob_vec.subvec(i, i), penalize.rows(block_indexes(i), block_indexes(i+1)-1)) - log_normal_density_matrix(b.rows(block_indexes(i), block_indexes(i+1)-1), arma::diagmat(eta.rows(block_indexes(i), block_indexes(i+1)-1)) / pow(s, 2), singular);
            ...
            if(i == 0 && icept == true){
                ...
                b.subvec(block_indexes(i), block_indexes(i+1) - 1) = betaprop + beta_hat_block;
                 ...
                 while (log_asymmetric_prior(beta_hat_block + betaprop, vglobal, prob_vec.subvec(i, i), penalize.rows(block_indexes(i), block_indexes(i+1)-1)) - log_normal_density_matrix(beta_hat_block + betaprop, arma::diagmat(eta.rows(block_indexes(i), block_indexes(i+1)-1)) / pow(s,2), singular) < ly){
