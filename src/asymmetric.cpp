...
blocked elliptical slice sampler, shark-fin prior
    ...
    */
    ...
    {   
        ...
        for(int i = 0; i < N_blocks; i ++){
            ...
            priorcomp = log_asymmetric_prior(b.rows(block_indexes(i), block_indexes(i+1)-1), vglobal, prob_vec.subvec(i, i)) - log_normal_density_matrix(b.rows(block_indexes(i), block_indexes(i+1)-1), eye<mat>(block_vec(i), block_vec(i)) / pow(s,2) / eta, singular);
            ...
            if(i == 0 && icept == true){
                ...
                b.subvec(block_indexes(i), block_indexes(i+1) - 1) = betaprop + beta_hat_block;
                 ...
                 while (log_asymmetric_prior(beta_hat_block + betaprop, vglobal, prob_vec.subvec(i, i)) - log_normal_density_matrix(beta_hat_block + betaprop, eye<mat>(block_vec(i), block_vec(i)) / pow(s,2) / eta, singular) < ly){
        ...
        if(icept == false){
            ...
            ratio = exp(log_asymmetric_prior(b.elem(penalize), vgprop, prob_vec) + log_normal_density(vgprop, 0.5, 100)  - log_asymmetric_prior(b.elem(penalize), vglobal, prob_vec) - log_normal_density(vglobal, 0.5, 100)  +log(vgprop) - log(vglobal));
        }else{
            // else, do not consider the intercept when evaluating prior function
            ratio = exp(log_asymmetric_prior(b.rows(1,p-1), vgprop, prob_vec.rows(1,p-1)) + log_normal_density(vgprop, 0.5, 100)  - log_asymmetric_prior(b.rows(1,p-1), vglobal, prob_vec) - log_normal_density(vglobal, 0.5, 100)  +log(vgprop) - log(vglobal));
        }
